...
router.delete("/deletar/:id", async (req, res) => {
  ...
  if(userById){
    ...
    try {
      await UserModel.deleteOne({ _id: userById.id });
      return res.status(200).json({ msg: "deletou" });
    } catch (error) {
      return res.status(401).json({ msg: "nao deletou" });
    }
  }else{
    res.status(400).json({msg: "não encontrou id"})
  }
...
router.get("/private/:id", checkToken, async (req, res) => {
  ...
  res.status(200).json({ msg: "Private route OK" });
});
...
router.patch("/update/:id", async (req, res) => {
  ...
  if(!undefined){
    ...
    res.status(401).json({ msg: "chamar id primeiro" });
  }
});
